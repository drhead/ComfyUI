...
def kl_optimal(model_sampling, steps):
    alpha_min = torch.tensor(torch.arctan(model_sampling.sigma_min), device=model_sampling.sigmas.device)
    alpha_max = torch.tensor(torch.arctan(model_sampling.sigma_max), device=model_sampling.sigmas.device)
    step_indices = torch.arange(steps+1, device=model_sampling.sigmas.device)
